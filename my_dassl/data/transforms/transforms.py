...
def build_transform(cfg, is_train=True, is_advtest=False, choices=None):
    ...
    if is_train:
        ...
        return _build_transform_adv_test()
    else:
...
def _build_transform_adv_test():
    print("Building transform_adv_test")
    tfm_test = []

    print("+ to torch tensor of range [0, 1]")
    tfm_test += [ToTensor()]

    tfm_test += [transforms.Lambda(lambda x: x - 0.5)]
